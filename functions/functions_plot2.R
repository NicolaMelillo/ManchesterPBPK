...
plotPBPK <- function(system.out.list, list.sim, names.sim){
  
  # initialize some useful variable outside the loop
  l.s <- length(list.sim)
  col <- gg_color_hue(l.s)
  
  lo.pbpk <- length(list.sim[[1]]$param.PBPK$comp_PBPK_names)
  df.pbpk.list <- list()
  p.pbpk       <- list()
  for(i in 1:lo.pbpk){
  ...
  lo.acat.1 <- length(list.sim[[1]]$param.PBPK$comp_ACAT_names[c(1:20, 33, 34)])
  df.acat.list <- list()
  p.acat.1     <- list()
  for(i in 1:lo.acat.1){
  ...
  for(jj in 1:l.s){
    ...
    param.PBPK <- list.sim[[jj]]$param.PBPK
    param_drug <- list.sim[[jj]]$param.PBPK$param_drug
    schedule   <- list.sim[[jj]]$schedule
    
    x.pbpk   <- system.out[param.PBPK$comp_PBPK_names]
    n.points <- length(system.out$time) 
    #lo.pbpk <- length(param.PBPK$comp_PBPK_names)
    
    # plot all the states dynamics
    for(i in 1:lo.pbpk){
      dfi.pbpk <- data.frame(time=system.out$time, mass=as.vector( x.pbpk[[param.PBPK$comp_PBPK_names[i]]]), n.sim = rep(names.sim[jj],n.points))
      df.pbpk.list[[i]] <- rbind(df.pbpk.list[[i]], dfi.pbpk)
    }
    
    
    # plot venous blood
    c.plasma <- system.out$venous_blood/param_drug["BP"]/param.PBPK$organ_v["venous_blood"]
    dfp_jj  <- data.frame(time=system.out$time, mass=c.plasma, n.sim = rep(names.sim[jj],n.points))
    dfp     <- rbind(dfp, dfp_jj)
    
    ### plot ACAT distribution ---------------------------------------------------------------
    
    x.acat  <- system.out[param.PBPK$comp_ACAT_names]
    
    x.acat.1  <- x.acat[c(1:20, 33, 34)]
    x.names.1 <- param.PBPK$comp_ACAT_names[c(1:20, 33, 34)]
    
    for(i in 1:lo.acat.1){
      dfi.acat <- data.frame(time=system.out$time, mass=as.vector( x.acat.1[[x.names.1[i]]]), n.sim = rep(names.sim[jj],n.points))
  ...
  for(i in 1:lo.pbpk){
    ...
    p.pbpk[[i]] <- ggplot() + geom_line(data=dfi, aes(x=time, y=mass, color=n.sim)) + 
      ...
      ggtitle(param.PBPK$comp_PBPK_names[i])
  }
  ...
  for(jj in 1:l.s){
    ...
    f_g <- sum(system.out[end.time,45:50])/dose_tot
    f_tot <- f_s + f_d + f_h + f_r + f_g
    
    names <- c(names, "f_solid", "f_diss", "f_gut", "f_clh", "f_clr", "total")
    fract <- c(fract, f_s, f_d, f_g, f_h, f_r, f_tot)
    idx   <- c(idx, rep(names.sim[jj],6))
    ord   <- c(ord, 1:6)
    
    ### absorbed fraction
    f_abs_s   <- unlist(system.out[end.time, 39:44]/dose_tot)
    f_abs_t   <- sum(f_abs_s)
    f_abs     <- c(f_abs, f_abs_s, f_abs_t)
    names_abs <- c(names_abs, names(f_abs_s), "total")
